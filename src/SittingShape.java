 ...
 * checking for the sitting shape.
 * 
 * @author Christopher Murray
 * @version 1.0 (April 2014)
 */
public class SittingShape extends Shape {

	/**
	 * Sets the initial position of the sitting shape.
	 */
	public void initialShape(int gridWidth) {
		// initial position of shape
		shapeX[0] = gridWidth / 2;
		shapeY[0] = 0;
	...
	/**
	 * Second rotation state, which for the sitting shape is it's initial
	 * position
	 */
	...
	private boolean checkRotationOne(int[][] grid) {
		if ((grid[shapeX[0] - 2][shapeY[0]] >= 2 && grid[shapeX[0] - 2][shapeY[0]] <= 8)
				|| (grid[shapeX[1] - 1][shapeY[1] - 1] >= 2 && grid[shapeX[1] - 1][shapeY[1] - 1] <= 8)
	...
	private boolean checkRotationTwo(int[][] grid) {
		if ((grid[shapeX[0] + 2][shapeY[0]] >= 2 && grid[shapeX[0] + 2][shapeY[0]] <= 8)
				|| (grid[shapeX[1] + 1][shapeY[1] + 1] >= 2 && grid[shapeX[1] + 1][shapeY[1] + 1] <= 8)
				|| (grid[shapeX[2]][shapeY[2]] >= 2 && grid[shapeX[2]][shapeY[2]] <= 8)
				|| (grid[shapeX[3] - 1][shapeY[3] + 1] >= 2 && grid[shapeX[3] - 1][shapeY[4] + 1] <= 8)) {
			return false;
		}
		return true;
	}
